...
import { getUserProfile } from "../lib/supabase-tables";
...
const Header = ({
  ...
  const [userInitial, setUserInitial] = useState<string>(userInitials);
    ...
    const fetchUserProfile = async () => {
      if (user) {
        try {
          ...
          if (data && !error) {
            const fullName = data.full_name || user.email || userName;
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="p-0">
              <Avatar>
                {userAvatarUrl ? (
                  <AvatarImage src={userAvatarUrl} alt={profileName} />
                ) : (
