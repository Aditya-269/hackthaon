...
const EmergencyPanel = ({
  isActive = true,
  contacts = [
    { name: "Mom", status: "seen" },
    { name: "Dad", status: "delivered" },
    { name: "Sister", status: "sent" },
    { name: "Emergency Services", status: "delivered" },
  ],
  ...
  const alertProgress = () => {
    const seen = contacts.filter((c) => c.status === "seen").length;
  ...
  return (
    <Card className="w-full max-w-[1400px] mx-auto bg-red-50/60 dark:bg-red-900/20 border-red-300/40 dark:border-red-700/40 shadow-2xl rounded-3xl overflow-hidden backdrop-blur-md">
            ...
            <div className="grid grid-cols-2 gap-2">
              {contacts.map((contact, index) => (
                ...
                >
                  ...
                  <Badge
                    variant={
                      contact.status === "seen" ? "default" : "secondary"
                    }
                    className={
                      contact.status === "seen"
                        ? "bg-green-500"
      ...
      <CardFooter className="flex justify-center p-6">
        <Button
          onClick={onMarkSafe}
          className="bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 dark:from-green-700 dark:to-green-600 dark:hover:from-green-800 dark:hover:to-green-700 text-white font-bold py-3 px-8 rounded-full flex items-center gap-2 shadow-md hover:shadow-lg transition-all duration-300"
        >
